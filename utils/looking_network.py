...
class LookingModel(nn.Module):
    ...
    def __init__(
        self, input_size, p_dropout=0.2, output_size=1, linear_size=256, num_stage=3
    ):
        """[summary]
...
class ResNet18_head(nn.Module):
    def __init__(self, device, fine_tune=True):
        ...
        self.net.fc = nn.Sequential(
            nn.Linear(in_features=512, out_features=1, bias=True), nn.Sigmoid()
        ).to(device)
        if fine_tune:
...
class ResNet50_head(nn.Module):
    def __init__(self, device, fine_tune=True):
        ...
        net.fc = nn.Sequential(
            nn.Linear(in_features=2048, out_features=1, bias=True), nn.Sigmoid()
        ).to(device)
        if fine_tune:
            for param in net.parameters():
                param.requires_grad = False
...
class LookingNet_late_fusion_18(nn.Module):
    ...
    def __init__(self, PATH, PATH_look, device, fine_tune=True):
        ...
        if fine_tune:
            self.looking_model.load_state_dict(
                torch.load(PATH_look, map_location=torch.device(device))
            )
    ...
    def forward(self, x):
        ...
        self.looking_model.linear_stages[2].bn2.register_forward_hook(
            get_activation("look")
        )
        ...
        out_final = torch.cat(
            (self.encoder_head(layer_resnet), self.relu(layer_look)), 1
        ).type(torch.float)
