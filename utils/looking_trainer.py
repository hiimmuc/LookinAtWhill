...
class Parser:
    ...
    def __init__(self, config):
        ...
        if self.general["device"] == "cpu":
            ...
            self.device = torch.device(
                "cuda:{}".format(self.general["device"]) if use_cuda else "cpu"
            )
    ...
    def get_model(self):
        ...
        if model_type == "joints":
            ...
            self.data_transform = transforms.Compose(
                [transforms.Resize((10, 30)), transforms.ToTensor()]
            )
            ...
            if self.model_type["trained_on"] == "JAAD":
                ...
                if backbone == "resnet18":
                    name_model_backbone = (
                        "_".join(["ResNet18_head", criterion.__class__.__name__, ""])
                        + ".p"
                    )
                else:
                    name_model_backbone = (
                        "_".join(["ResNet50_head", criterion.__class__.__name__, ""])
                        + ".p"
                    )
            path_output_model_backbone = os.path.join(
                self.general["path"], self.model_type["trained_on"], "Heads"
            )
            path_backbone = os.path.join(
                path_output_model_backbone, name_model_backbone
            )

            path_output_model_joints = os.path.join(
                self.general["path"], self.model_type["trained_on"], "Joints"
            )
            path_model_joints = os.path.join(
                path_output_model_joints, name_model_joints
            )
            print(path_backbone)
            ...
            if fine_tune:
                if not os.path.isfile(path_backbone):
                    print(
                        "ERROR: Heads model not trained, please train your heads model first"
                    )
                    exit(0)
                if not os.path.isfile(path_model_joints):
                    print(
                        "ERROR: Joints model not trained, please train your joints model first"
                    )
                    exit(0)
            fusion_type = self.general["fusion_type"]
            if backbone == "resnet18":
                if fusion_type == "early":
                    model = LookingNet_early_fusion_18(
                        path_backbone, path_model_joints, self.device, fine_tune
                    )
                else:
                    model = LookingNet_late_fusion_18(
                        path_backbone, path_model_joints, self.device, fine_tune
                    )
            else:
                if fusion_type == "early":
                    model = LookingNet_early_fusion_50(
                        path_backbone, path_model_joints, self.device, fine_tune
                    )
                else:
                    model = LookingNet_late_fusion_50(
                        path_backbone, path_model_joints, self.device, fine_tune
                    )
        else:
            fine_tune = self.model_type.getboolean("fine_tune")
            ...
            if fine_tune:
                if not os.path.isfile(path_model_joints):
                    ...
                    exit(0)
            fusion_type = self.general["fusion_type"]
            model = LookingNet_early_fusion_eyes(
                path_model_joints, self.device, fine_tune
            )
            ...
            if fine_tune and model_type == "heads":
                if backbone == "alexnet":
                    optimizer = torch.optim.SGD(
                        model.net.classifier.parameters(), lr=self.lr, momentum=0.9
                    )
                else:
                    optimizer = torch.optim.SGD(
                        model.net.fc.parameters(), lr=self.lr, momentum=0.9
                    )
            else:
                optimizer = torch.optim.SGD(
    ...
    def get_data(self, data_type):
        ...
        if self.multi_dataset:
            ...
            paths_txt = [
                os.path.join(self.data_args["path_txt"], "splits_" + data_name.lower())
            ...
            path_txt = os.path.join(
                self.data_args["path_txt"], "splits_" + data_type.lower()
            )
    ...
    def parse(self):
        self.model, self.criterion, self.optimizer, self.data_transform = (
            self.get_model()
        )
        if self.multi_dataset:
            ...
            self.path_output = os.path.join(
                self.general["path"], names, self.model_type["type"].title()
            )
            self.dataset_train, self.dataset_val = self.get_data(
                self.multi_args["train_datasets"]
            )
            self.path_output = os.path.join(
                self.general["path"], names, self.model_type["type"].title()
            )
...
class Evaluator:
    ...
    def __init__(self, parser):
        ...
        if os.path.isfile(self.parser.path_model):
            ...
            print(
                "ERROR : Model file doesnt exists, please train your model first or review your parameters"
            )
            exit(0)
    ...
    def evaluate(self):
        ...
        if self.height_ == False:
            ...
            print(
                "Evaluation on {} | acc:{:.1f} | ap:{:.1f}".format(
                    data_to_evaluate, acc, ap * 100
                )
            )
        else:
            ap, acc, ap_1, ap_2, ap_3, ap_4, distances = data_test.evaluate(
                self.parser.model, self.parser.device, 10, True
            )

            print("Distances : ", np.mean(distances, axis=0))
            print(
                "Ap Far : {:.1f} | Middle 1 : {:.1f} | Middle_2 : {:.1f} | Close :{:.1f}".format(
                    ap_1 * 100, ap_2 * 100, ap_3 * 100, ap_4 * 100
                )
            )
...
class Trainer:
    ...
    def get_sampler(self, concat_dataset):
        weigths = 1.0 / torch.Tensor([len(data) for data in concat_dataset])
        di = {}
        ...
        sampler = WeightedRandomSampler(
            torch.Tensor(weigths_samples), len(weigths_samples)
        )
        return sampler
    ...
    def train(self):
        ...
        if self.parser.multi_dataset:
            ...
            if self.parser.weighted:
                ...
                train_loader = DataLoader(
                    concat_dataset, batch_size=self.parser.batch_size, sampler=sampler
                )
            else:
                train_loader = DataLoader(
                    concat_dataset, batch_size=self.parser.batch_size, shuffle=True
                )
        else:
            train_loader = DataLoader(
                self.parser.dataset_train,
        ...
        for epoch in range(self.parser.epochs):
            ...
            print("")
            print(
                "Epoch {} | mAP_val : {} | mAcc_val :{}".format(
                    epoch + 1, ap_val, acc_val
                )
            )
            ...
            if self.get_grads:
                ...
                for joints, labels in DataLoader(
                    self.parser.dataset_train, batch_size=len(self.parser.dataset_train)
                ):
                    joints, labels = joints.to("cpu"), labels.to("cpu").type(
                        torch.float
                    )
                    break
        ...
        if self.get_grads:
            ...
            ax = sns.heatmap(
                grads_magnitude_,
                linewidth=0.5,
                yticklabels=y_labels[:17],
                xticklabels=list(range(1, self.parser.epochs + 1)),
                vmin=0,
                vmax=1,
            )
            ...
            ax.set_xticklabels(
                [str((i + 1) * 5) for i in range(int(self.parser.epochs / 5))],
                rotation=0,
            )
    ...
    def eval_epoch(self, best_ap, best_ac):
        ...
        if self.parser.multi_dataset:
            ...
            for data in self.parser.dataset_val:
                aps, accs = data.evaluate(
                    self.parser.model,
                    self.parser.device,
                    it=self.parser.eval_it,
                    heights_=False,
                )
                tab_ap.append(aps)
                tab_acc.append(accs)
            ...
            aps, accs = self.parser.dataset_val.evaluate(
                self.parser.model, self.parser.device, it=self.parser.eval_it
            )
